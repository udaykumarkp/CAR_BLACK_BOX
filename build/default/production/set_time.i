...
unsigned char set_tm[9];

void set_time(unsigned char key , unsigned char * reset)
{
    static int i = 0 , curs_flag ;
    if(*reset == 0x0F)
    {
        for(int i=0;i<8;i++)
            set_tm[i] = '0';
    ...
    if(set_tm[0] == '2')
          set_tm[1] = '0' + (set_tm[1]-'0') % 4;
    clcd_print(set_tm,(0xC0 + 0));
    if(curs_flag = (!curs_flag))
        clcd_putch(0xFF , (0xC0 + i));
}
...
void save_time(void)
{
    ...
    write_ds1307(0x02,data);
    data = 0;
    data = data| ((set_tm[3]-'0')<<4) | (set_tm[4] - '0');
    write_ds1307(0x01,data);
    data = 0;
    data = data| ((set_tm[6]-'0')<<4) | (set_tm[7] - '0');
    write_ds1307(0x00,data);
}
